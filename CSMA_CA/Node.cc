...
void Node::finish()
{
    ...
    double endTime = SIMTIME_DBL(simTime());    // 网络运行时间
    if (this->getId() == 5)
    {
        ...
        DataThroughput = (double) (totalSentDataPackets * this->datapacketSize) /
                (double) (runningTime * this->networkSpeed);
        cout << "running time = " << runningTime << ", data throughput = " <<
                DataThroughput << endl;
        cout << "The total sent data packets = " << totalSentDataPackets << endl;
        cout << "The total number of data node = " << this->dataNodeNum << endl;
        cout << "The normalized data throughput = " << DataThroughput << endl;
    }
}
...
void Node::networkInitialization()
{
    ...
    this->lossPackets = 0;  // 丢包个数，当一个数据包发送次数超过retryLimit时将该包丢弃
...
void Node::dataPacketAccess()
{
    ...
    // cout << "Node " << this->getId() << "dataPacketAccess" << endl;
    Busy = 0;
    IsDataQueueEmpty();
}
...
void Node::DataPacketTransmission()
{
    ...
    if (first_time_data_transmission == 0)
    {
        ...
        for (int i = 0; i < datanodev.size(); i++)
        {
            if (datanodev[i]->transmission_status == 1 && datanodev[i]->getId() !=
                    this->getId())
            {
                this->retryCounter++;
                // this->backoffStage++;
    ...
    else
    {
        for (int i = 0; i < datanodev.size(); i++)
        {
            if (datanodev[i]->CW_counter == 0 && datanodev[i]->getId() !=
                     this->getId())
            {
                this->retryCounter++;
                this->backoffStage++;
